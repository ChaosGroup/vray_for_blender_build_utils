...
compile_OIIO() {
  ...
  if [ ! -d $_inst ]; then
    ...
    # For CentOS
    # NOTE: Manually build static tiff and openjpeg
    # (disable bin compilation of openjpeg via CMakeLists.txt editing)
    #
    # Have no idea what openjpeg it wants; 2.1 doesn't work
    cmake_d="$cmake_d -DUSE_OPENJPEG=OFF"
    cmake_d="$cmake_d -DTIFF_INCLUDE_DIR=/opt/lib/tiff-3.9.7/include"
    cmake_d="$cmake_d -DTIFF_LIBRARY=/opt/lib/tiff-3.9.7/lib/libtiff.a"
    cmake_d="$cmake_d -DOPENJPEG_INCLUDE_DIR=/opt/lib/openjpeg-2.1/include/openjpeg-2.1"
    cmake_d="$cmake_d -DOPENJPEG_OPENJPEG_LIBRARY=/opt/lib/openjpeg-2.1/lib/libopenjp2.a"
...
compile_OSL() {
  ...
  if [ ! -d $_inst ]; then
    ...
    # NOTE: Edit CMakeLists.txt and disable src/test*
    #
    cmake_d="$cmake_d -DBoost_DIR=/opt/lib/boost"
    cmake_d="$cmake_d -DBoost_INCLUDE_DIR=/opt/lib/boost/include"
    cmake_d="$cmake_d -DBoost_LIBRARY_DIR_RELEASE=/opt/lib/boost/lib"
    cmake_d="$cmake_d -DBoost_REGEX_LIBRARY_RELEASE=/opt/lib/boost/lib/libboost_regex.a"
    cmake_d="$cmake_d -DBoost_SYSTEM_LIBRARY_RELEASE=/opt/lib/boost/lib/libboost_system.a"
    cmake_d="$cmake_d -DBoost_THREAD_LIBRARY_RELEASE=/opt/lib/boost/lib/libboost_thread.a"
    cmake_d="$cmake_d -DBoost_FILESYSTEM_LIBRARY_RELEASE=/opt/lib/boost/lib/libboost_filesystem.a"

    cmake_d="$cmake_d -DILMBASE_HALF_LIBRARY_RELEASE=/opt/lib/openexr/lib/libHalf.a"
    cmake_d="$cmake_d -DILMBASE_IEX_LIBRARY_RELEASE=/opt/lib/openexr/lib/libIex.a"
    cmake_d="$cmake_d -DILMBASE_ILMTHREAD_LIBRARY_RELEASE=/opt/lib/openexr/lib/libIlmThread.a"
    cmake_d="$cmake_d -DILMBASE_IMATH_LIBRARY_RELEASE=/opt/lib/openexr/lib/libImath.a"
