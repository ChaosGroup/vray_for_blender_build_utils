	...
	def compile(self):
		...
		if self.dev_static_libs:
			if distr_info['short_name'] == 'centos':
				...
				cmake.append("-DPYTHON_VERSION=%s" % PYTHON_VERSION)
				cmake.append("-DPYTHON_ROOT_DIR=/opt/python-%s" % PYTHON_VERSION)
				cmake.append("-DPYTHON_LIBRARY=/opt/python-%s/lib/libpython%sm.a" % (PYTHON_VERSION, PYTHON_VERSION))
				cmake.append("-DPYTHON_LIBPATH=/opt/python-%s/lib" % PYTHON_VERSION)
				cmake.append("-DPYTHON_LIBRARIES=/opt/python-%s/lib" % PYTHON_VERSION)
				cmake.append("-DPYTHON_INCLUDE_DIR=/opt/python-%s/include/python%sm" % (PYTHON_VERSION, PYTHON_VERSION))
				cmake.append("-DPYTHON_INCLUDE_CONFIG_DIR=/opt/python-%s/include/python%sm" % (PYTHON_VERSION, PYTHON_VERSION))
				cmake.append("-DPYTHON_NUMPY_PATH=/opt/python-%s/lib/python%s/site-packages" % (PYTHON_VERSION, PYTHON_VERSION))
